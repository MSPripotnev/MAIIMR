...
class FSM: #Finite State Machine
    def __init__(self):
        ...
        self.getState("S1").transitions={"S1":"Right", "S2":"S2", "S3":"S3"}
        self.getState("S2").transitions={"S1":"S1", "S2":"Stay", "S3":"S3"}
        self.getState("S3").transitions={"S1":"S1", "S2":"S2", "S3":"Left"}
        self.getState("Left").transitions={"S1":"S1", "S2":"S2", "S3":"S3"}
        self.getState("Stay").transitions={"S1":"S1", "S2":"S2", "S3":"S3"}
        self.getState("Right").transitions={"S1":"S1", "S2":"S2", "S3":"S3"}
    ...
    def draw(self, screen, dx=0, dy=0):
        for s in self.states:
            ...
            for k in s.transitions.keys():
                s1=s
                s2=self.getState(s.transitions[k])
    ...
    def makeTransition(self, signal):
        for s in self.states:
            if s.isActive:
                s.isActive=False
                stateName=s.transitions[signal]
                s2=self.getState(stateName)
                s2.isActive=True
                break
