...
class Robot:
    ...
    def goto(self, x, y):
        ...
        self.vRot=0.3*gamma
        self.vLin=50
    def control(self, objs, v_max, v_min, rot_max):
...
def main():
    ...
    for i in range(0, max_attempt_v_min):
        v_min = round(min_v_min + i * (max_v_min - min_v_min) / max_attempt_v_min, 4)
        if logFile is not None:
            logFile.write("")
        for j in range(0, max_attempt_v_max):
            v_max = round(min_v_max + j * (max_v_max - min_v_max) / max_attempt_v_max, 4)
            if logFile is not None:
                logFile.write("")
            for k in range(0, max_attempt_r_max):
                rot_max = round(min_r_max + k * (max_r_max - min_r_max) / max_attempt_r_max, 4)
                tSim=0
                nColl=0
                tInd = 0
                nDet = 0
                ...
                while True:
                    if tSim>=60 and logFile is not None:
                        ...
                        logFile.write(f"v_max = {v_max}, v_min = {v_min}, rot_max = {rot_max}\n" +
                                    f"dist = {r.coveredDist}, nColl = {nColl}, nDet = {nDet}, Q = {Q}\n")
                        break
                    ...
                    r.control(objs, v_max, v_min, rot_max)
                    ...
                    drawText(screen, f"i={i},j={j},k={k}", 5, 130)
